/* =============================
   ...
   Converted into a C++ object by John Doug Reynolds, March 1998
...
static void    sort(double *x,int *ii,int n);
...
void	ReimannPhaser::init( void )
{
   ...
   times = yy = wei = fit = 0; /* wrong, should be y */
...
void	ReimannPhaser::free( void )
{
   ...
   if ( yy ) delete [] yy; /* this is wrong, should be y */
   if ( wei ) delete [] wei;
   if ( fit ) delete [] fit;
...
void	ReimannPhaser::load( const ClifParameters &par )
{
   ...
   SUBP = par.get( "ReimannPhaser.SUBP", 20.0 );
   FULL = par.get( "ReimannPhaser.FULL", 1 );
   MINP = par.get( "ReimannPhaser.MINP", 0.3 );
   ...
   SHOW = par.get( "ReimannPhaser.SHOW", 10 );
}
